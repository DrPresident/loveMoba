...
function Hero:loadSprite(path, delay)
    ...
    self.sprite.frameWidth = 32 --97 / 3  --math.floor(self.sprite.image:getWidth() / 3)
    self.sprite.frameHeight = 128 / 4 --math.floor(self.sprite.image:getHeight() / 4)

    self.sprite.grid = anim8.newGrid(self.sprite.frameWidth, self.sprite.frameHeight, self.sprite.image:getWidth(), self.sprite.image:getWidth())
...
function Hero:anim()
    ...
    elseif math.abs(self:X() - self.desX) < math.abs(self:Y() - self.desY) then
        if (self:Y() - self.desY) > 0 then
            return self.frontAnim --self.backAnim
